...
# Rutas
file_path <- "input/muertesAlzhimer.csv"
datos_dir <- "input"
out_dir   <- "results"
...
# 2. Leer muertes Alzheimer y filtrar
datos_MA <- read_csv(file_path, show_col_types = FALSE) %>%
  ...
  filter(!is.na(country), !is.na(year), !is.na(numero_muertes)) #Elimina filas donde alguna de estas tres columnas sea NA
...
datos_combinados <- inner_join(
  ...
  arrange(country, year) #PARA ORDENAR
...
# Exportar resultado
write_csv(datos_combinados, file.path(out_dir, "muertesAlzheimer_HgPb.csv"))
cat("Archivo guardado en:", file.path(out_dir, "muertesAlzheimer_HgPb.csv"), "\n")
