...
El análisis del impacto sistémico para el Parkinson revela una jerarquía de toxicidad similar a la del Alzheimer, aunque con diferencias notables en la cantidad de genes afectados. De los `r sum(df_villanos_parkinson$Genes_Afectados)` genes de riesgo válidos identificados, se concluye que:
...
Este estudio está limitado a los datos que conseguimos,proceden de fuentes y años distintos con cobertura variable por país, son agregados a nivel poblacional y no incluyen datos individuales ni biomarcadores sistemáticos, por lo que no es un trabajo completo y sus resultados deben interpretarse con mucha cautela. A pesar de estas limitaciones observamos que los niveles poblacionales de plomo (Pb) han disminuido con el tiempo mientras que los de mercurio (Hg) se mantienen más estables; la mortalidad estimada por Alzheimer y Parkinson no baja en la misma medida y los análisis ecológicos muestran asociaciones contradictorias (por ejemplo, una aparente relación negativa entre Pb y muertes) que probablemente se deben a sesgos, diferencias temporales, heterogeneidad en las mediciones y factores confusores, no a un efecto protector. A nivel molecular hay un solapamiento importante entre genes que interactúan con Pb/Hg y genes asociados por GWAS (aproximadamente 1.069 para Alzheimer y 174 para Parkinson en nuestros datos), y los derivados orgánicos del mercurio afectan a muchos genes de riesgo, lo que indica puntos de interés biológico. Sin embargo, por el diseño agregado, la cobertura limitada y la mezcla de tipos de evidencia (in vitro, anotaciones, text‑mining), no podemos afirmar causalidad; por tanto, no se debe interpretar la correlación negativa como protección. Recomendamos priorizar estudios individuales y longitudinales con biomarcadores y datos genéticos, validar experimentalmente los compuestos villanos identificados y mantener o reforzar las medidas para reducir la exposición a Pb y Hg en la población. En resumen: el estudio, aunque restringido por los datos disponibles, identifica señales y prioridades claras para investigación y prevención, pero no prueba relaciones causales.
