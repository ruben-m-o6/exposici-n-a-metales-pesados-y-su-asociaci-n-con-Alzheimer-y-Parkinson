...
```{r genes-villanos-alzheimer, echo=FALSE, message = FALSE, warning = FALSE, fig.width= 9, fig.height=6}
library(readr)
library(dplyr)
library(ggplot2)
...
# 2. Filtrar solo los "Villanos" (Genes de Riesgo: OR > 1)
df_villanos_alz <- df_alzheimer %>%
  filter(orValue > 1) %>%
...
# 3. Generar el Gráfico de Barras (Ranking de Villanos)
plot_villanos_alz <- ggplot(df_villanos_alz, 
  ...
  labs(
    title = "Ranking de Villanos: Compuestos Tóxicos que Afectan a Genes de Riesgo (Alzheimer)",
...
En conclusión, el Ranking de Villanos establece una jerarquía donde los derivados de Mercurio Orgánico representan la mayor amenaza en términos de amplitud de genes de riesgo afectados, informando nuestra hipótesis de que estos compuestos son los principales impulsores de la sinergia tóxica.
...
library(dplyr)
library(ggplot2)
library(ggrepel) # Para evitar que se superpongan las etiquetas
library(plotly) # Para la interactividad
# library(ggrepel) # No se necesita para plotly

# Cargar el archivo de datos limpio de Alzheimer (asumiendo que está en inputs/)
source("alzheimerMetalesGenes.R")

# CORRECTIVO DE COLUMNAS: Aseguramos que las columnas clave se llamen correctamente.
df_alzheimer <- df_alzheimer %>%
  rename(
    mappedGenes = any_of("mappedGenes"),      
    orValue = any_of("orValue"),              
    ChemicalName = any_of("ChemicalName")     
  ) %>%
  rename(
    ChemicalName = any_of(c("chemicalName", "Chemical.Name", "Chemical name", "Chemical_Name")),
    mappedGenes = any_of(c("MappedGenes", "mapped_genes", "GeneSymbol")),
    orValue = any_of(c("orvalue", "OddsRatio"))
  )

# 1. Preparación de datos para el Plot de Impacto
df_plot <- df_alzheimer %>%
  # Calcular valores transformados para los ejes
  mutate(
    neg_log_pvalue = -log10(pValue),
    log_or_value = log10(orValue),
    # Creamos un texto personalizado para el tooltip
    Tooltip_Text = paste(
      "Gen:", mappedGenes, 
      "<br>Químico:", ChemicalName, 
      "<br>OR:", round(orValue, 3),
      "<br>P-Value:", format.pval(pValue, digits = 2),
      "<br>Acción:", InteractionActions
    )
  ) %>%
  
  # Clasificar las interacciones en zonas de riesgo 
  mutate(
    Zona_Impacto = case_when(
      orValue > 1 & InteractionActions == "increase" ~ "Tormenta Perfecta (Sinergia Tóxica)",
      orValue < 1 & InteractionActions == "decrease" ~ "Defensa Rota (Protección Anulada)",
      orValue > 1 & InteractionActions == "decrease" ~ "Riesgo Mitigado",
      orValue < 1 & InteractionActions == "increase" ~ "Protección Reducida",
      TRUE ~ "Neutro"
    )
  )

# 2. Crear el Scatter Plot base (ggplot)
plot_base <- ggplot(df_plot, aes(x = log_or_value, y = neg_log_pvalue, color = Zona_Impacto, text = Tooltip_Text)) + # Usamos 'text' para el tooltip
  ...
  # Etiquetado y Temas
  labs(
    title = "Scatter Plot de Impacto: Riesgo Genético vs. Interacción con Metales (Alzheimer)",
    subtitle = "El Cuadrante del Peligro (Interactivo)",
    x = expression(log[10] * "(Odds Ratio - OR)"),
    y = expression("-log"[10] * "(P-Value)"),
    color = "Zona de Impacto"
  ) +
  theme_minimal() +
  scale_color_manual(values = c(
    "Tormenta Perfecta (Sinergia Tóxica)" = "#CC3333",          
    "Defensa Rota (Protección Anulada)" = "#FF9933",            
    "Riesgo Mitigado" = "skyblue",
    "Protección Reducida" = "green"
  ))

# 3. Convertir a interactivo usando ggplotly
ggplotly(plot_base, tooltip = "text")
```

Este Scatter Plot de Impacto permite priorizar el estudio de los genes más críticos en las zonas de peligro:

&middot; Sinergia Tóxica ("Tormenta Perfecta"): Los genes en el cuadrante de Alto Riesgo (OR >1)  y Alta Fiabilidad (P-Value bajo) representan el escenario de sinergia tóxica. Al analizar los puntos más extremos en la derecha superior, se identifican genes cruciales:

- Gen Crítico 1: El gen [APOE (3.21)] se destaca como el gen de riesgo más potente al interactuar con [Methylmercuric Chloride]. Su posición extrema en la gráfica indica que la exposición a este químico amplifica significativamente la susceptibilidad genética.

- Gen Crítico 2: El gen [APOE (3.1)] también se encuentra en esta zona de peligro al interactuar con [Methylmercuric Chloride], confirmando que una de las variantes más conocidas para la EA es una diana molecular directa de los metales pesados.

&middot; Defensa Rota ("Protección Anulada"): En el cuadrante de Factor Protector (OR < 1) y Alta Fiabilidad, se encuentran los genes cuya defensa es eliminada por el metal:

- Gen Protector Clave: Se identifica el gen [APOE (0.502)] como un punto crítico. La interacción con [Methylmercuric Chloride] disminuye su expresión protectora, lo que constituye una vulnerabilidad adquirida por el medio ambiente.

El análisis integrado de las interacciones genéticas y los compuestos tóxicos confirma la hipótesis de la Doble Vulnerabilidad en la patogénesis del Alzheimer:

- Vulnerabilidad Genética (GWAS): El riesgo existe y es cuantificable a través del Odds Ratio.

- Amplificación Ambiental: Los genes definidos como "villanos" genéticos son también las principales dianas moleculares de los metales pesados. El Methylmercuric Chloride (el agente de mayor impacto sistémico) actúa sobre una amplia red de genes de riesgo, generando el escenario de Sinergia Tóxica que amplifica la enfermedad.

En conclusión, el análisis confirma que la exposición a metales pesados no es un factor de riesgo independiente para el Alzheimer, sino un catalizador que potencia la susceptibilidad genética preexistente.
...
```{r genes-villanos-parkinson, echo=FALSE, message = FALSE, warning = FALSE, fig.width=9, fig.height=6}
library(readr)
library(dplyr)
library(ggplot2)
...
# 2. Filtrar solo los "Villanos" (Genes de Riesgo: OR > 1)
df_villanos_parkinson <- df_parkinson %>%
  filter(orValue > 1) %>%
  # Contar la cantidad de genes de riesgo únicos afectados por cada químico
  group_by(ChemicalName) %>%
  summarise(
...
# 3. Generar el Gráfico de Barras (Ranking de Villanos)
plot_villanos_parkinson <- ggplot(df_villanos_parkinson,
                                   aes(x = reorder(ChemicalName, Genes_Afectados),
                                       y = Genes_Afectados,
                                       fill = Genes_Afectados)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_text(aes(label = Genes_Afectados), vjust = -0.5, size = 4) +
  labs(
    title = "Ranking de Villanos: Compuestos Tóxicos que Afectan a Genes de Riesgo (Parkinson)",
    subtitle = "Número de Genes de Riesgo (OR > 1) con Interacción Demostrada por Compuesto Químico",
    x = "Compuesto Químico (Hg/Pb)",
    y = "Número de Genes de Riesgo Afectados"
  ) +
...
# Imprimir la gráfica para que se muestre en el R Markdown
print(plot_villanos_parkinson)
```

El análisis del impacto sistémico para el Parkinson revela una jerarquía de toxicidad similar a la del Alzheimer, aunque con diferencias notables en la cantidad de genes afectados. De los [CANTIDAD TOTAL DE GENES CON OR VÁLIDO] genes de riesgo válidos identificados, se concluye que:
...
```{r genes-interaccion-parkinson, echo=FALSE, message = FALSE, warning = FALSE, fig.width=9, fig.height=6}
library(readr)
library(dplyr)
library(ggplot2)
library(plotly) # Para la interactividad
# La función source("parkinsonMetalesGenes.R") ya fue ejecutada en el chunk anterior y creó df_parkinson

# 1. Preparación de datos para el Plot de Impacto
df_plot_park <- df_parkinson %>%
  # Calcular valores transformados para los ejes
  mutate(
    neg_log_pvalue = -log10(pValue),
    log_or_value = log10(orValue),
...
# 2. Crear el Scatter Plot base (ggplot)
plot_base_park <- ggplot(df_plot_park, aes(x = log_or_value, y = neg_log_pvalue, color = Zona_Impacto, text = Tooltip_Text)) +
  geom_point(alpha = 0.7, size = 3) +
  ...
  # Etiquetado y Temas
  labs(
    title = "Scatter Plot de Impacto: Riesgo Genético vs. Interacción con Metales (Parkinson)",
    subtitle = "El Cuadrante del Peligro (Interactivo)",
    x = expression(log[10] * "(Odds Ratio - OR)"),
    y = expression("-log"[10] * "(P-Value)"),
    color = "Zona de Impacto"
...
# 3. Convertir a interactivo usando ggplotly
ggplotly(plot_base_park, tooltip = "text")
```

El Scatter Plot de Impacto para el Parkinson permite identificar los puntos calientes de la interacción gen-metal, priorizando los genes de alta fiabilidad.

&middot; Sinergia Tóxica ("Tormenta Perfecta"): Los puntos extremos en la Derecha Superior ({OR} > 1 y ({P}-Value bajo) indican los siguientes hallazgos de alto impacto:

- Gen Crítico 1: El gen [SNCA (1.33)] se destaca como el gen de riesgo más potente, interactuando con [Mercuric Bromide]. Su fuerte correlación es una evidencia directa de que la exposición amplifica la susceptibilidad a la EP.

- Gen Crítico 2: El gen [SNCA (1.32)], otra variante conocida de la EP, también se posiciona en la zona de peligro al interactuar con [Mercuric Bromide].

&middot; Defensa Rota ("Protección Anulada"): Los puntos en la Izquierda Superior (OR < 1 y (P-Value bajo) revelan la vulnerabilidad de los genes protectores:

- Gen Protector Clave: El gen [CARS2] es un punto crítico. La interacción con [Methlylmercuric chloride] disminuye su expresión protectora, lo que constituye una vulnerabilidad adquirida por el medio ambiente.

Como conclusión, la exposición a derivados orgánicos del mercurio y plomo no es un factor de riesgo independiente, sino un catalizador que potencia la susceptibilidad genética preexistente a la EP. El control de la exposición a estos contaminantes es fundamental para mitigar el riesgo de Parkinson en individuos genéticamente susceptibles.